-- COMPLETE DATABASE RLS SECURITY FOR GOVERNMENT COMPLIANCE
-- This script enables RLS on ALL tables visible in the Supabase dashboard
...
ALTER TABLE public.subsector_metrics ENABLE ROW LEVEL SECURITY;
...
-- Core VOFC data - Most sensitive, admin/SPSA only
...
-- Sector data - Authenticated users only
...
CREATE POLICY "secure_sector_metrics" ON public.sector_metrics
    FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "secure_subsector_metrics" ON public.subsector_metrics
    FOR ALL USING (auth.role() = 'authenticated');
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.sector_metrics TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.subsector_metrics TO authenticated;
