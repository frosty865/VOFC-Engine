...
print("TABLE PARSER: 3-Column Format")
print("=" * 50)
    ...
    while i < len(lines):
        ...
        if "The facility" in line:
            ...
            if len(parts) > 1:
                ...
                # Collect OFCs - each bullet point is a separate OFC
                while i < len(lines):
                    next_line = lines[i]
                    ...
                    if next_line.startswith('•'):
                        ...
                        # Clean up the OFC text (remove citation numbers at the end)
                        ofc_text = re.sub(r'\s+\d{2,3}\s*$', '', ofc_text)
...
# Create sample source links for demonstration
sample_source_links = 0
import random

for ofc in ofcs.data:
    # Pick a random source for each OFC
...
print("\n3-column table structure parsed with sources linked!")
