...
import type { Extracted } from "../parsers/vofcParser.js";

export async function persistExtraction(aid: string, ex: Extracted) {
  ...
  for (const l of ex.links) {
    ...
    if (vuln && ofc) links.push({ vulnerability_id: vuln.id, ofc_id: ofc.id, link_type: "direct", confidence_score: 1.0 });
  }
  if (links.length) {
