...
print("PDFPLUMBER PARSER: Proper Table Extraction")
print("=" * 50)
                ...
                for row_num, row in enumerate(table[1:], 1):
                    if len(row) >= 3 and row[0] and row[1] and row[2]:
                        ...
                        if ofcs_text:
                            ...
                            # Look for various bullet characters
                            bullet_pattern = r'[•\u2022\u2023\u25E6\u2043\u204C\u204D\u2219\u22C5\u00B7]'
                            ofc_lines = re.split(bullet_pattern, ofcs_text)
                            
                            for ofc_line in ofc_lines:
                                ofc_line = ofc_line.strip()
                                if ofc_line and len(ofc_line) > 10:
                                    # Remove citation numbers at the end
                                    ofc_line = re.sub(r'\s+\d+\s*$', '', ofc_line)
                                    ofc_line = ofc_line.strip()
                                    
                                    if ofc_line:
                                        ofcs.append(ofc_line)
                                        try:
                                            print(f"      Added OFC: {ofc_line[:50]}...")
...
print("\nPdfPlumber properly extracted table structure!")
